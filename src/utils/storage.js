...
const accentThemeSchema = z.union([
    z.literal('aurora'),
    z.literal('forest'),
    z.literal('sunset'),
    z.literal('midnight')
]);
const surfaceThemeSchema = z.union([
    z.literal('indigo'),
    z.literal('midnight'),
    z.literal('slate'),
    z.literal('charcoal')
...
function normalizeLifeReflections(raw) {
    const normalized = {};
    if (!raw) {
        return normalized;
    }
...
function normalizeLifeWins(raw) {
    const normalized = {};
    if (!raw) {
        return normalized;
    }
...
function normalizeDaySummaries(raw) {
    if (!raw) {
        return {};
    }
    const normalized = {};
...
function normalizePreferences(preferences) {
    const { accentTheme = 'aurora', surfaceTheme = 'indigo', dayFulfillmentThreshold = 40, weekFulfillmentTarget = 3, reminderLeadMinutes = 15, defaultReminderTime } = preferences ?? {};
    return {
        reminderLeadMinutes,
        defaultReminderTime,
        accentTheme,
